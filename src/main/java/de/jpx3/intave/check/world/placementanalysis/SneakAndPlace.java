...
//@Reserved
public final class SneakAndPlace extends PlayerCheckPart<PlacementAnalysis> {
	...
	public SneakAndPlace(User user, PlacementAnalysis parentCheck) {
...
//      player.sendMessage(sneakInThisTick + "("+startSneakInThisTick+","+stopSneakInThisTick+")/" + placedInThisTick);
			if (placedInThisTick) {
				...
				if (!suspiciousSneaking && violationLevel > 0) {
					...
					violationLevel += diff > 1 ? 0.1 : 0.5;
//          player.sendMessage(ChatColor.YELLOW + "Sneak start -> Place: " + diff + " last duration: " + lastSneakDuration);
				}
			}
			if (sneakChangedInThisTick) {
				...
				if (!suspiciousSneaking && violationLevel > 0) {
					...
					violationLevel += diff > 1 ? 0.1 : 0.75;
//          player.sendMessage(ChatColor.YELLOW +"Place -> Sneak start: " + diff + " last duration: " + lastSneakDuration);
				}
			}
		}
